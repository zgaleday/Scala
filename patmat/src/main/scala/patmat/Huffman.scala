...
object Huffman {
   ...
   */
    def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
      def traverse(subTree: CodeTree, subBits: List[Bit]): List[Char] = subTree match {
        case Leaf(char, weight) => if (subBits.nonEmpty) char :: traverse(tree, subBits) else List(char)
        case Fork(l, r, c, w) => if (subBits.head == 0) traverse(l, subBits.tail) else traverse(r, subBits.tail)
      }
      traverse(tree, bits)
    }
   ...
   */
    def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
      def traverse(subTree: CodeTree, subChars: List[Char]): List[Bit] = subTree match {
        case Leaf(char, weight) => if (subChars.tail.nonEmpty) traverse(tree, subChars.tail) else Nil
        case Fork(l, r, c, w) =>
          if (chars(l).contains(subChars.head)) 0 :: traverse(l, subChars)
          else 1 :: traverse(r, subChars)
   ...
   */
    def convert(tree: CodeTree): CodeTable = {
      def traverse(subTree: CodeTree,bits: List[Bit]): List[(Char, List[Bit])] = subTree match {
        case Fork(l, r, c, w) => traverse(l, 0 :: bits) union traverse(r, 1 :: bits)
        case Leaf(c, w) => List((c, bits.reverse))
      }
      traverse(tree, Nil)
    }
