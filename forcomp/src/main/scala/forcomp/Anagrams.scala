...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = {
    ...
    val occurMap = charList groupBy ((element: Char) => element) map{ case (c, l) => (c, l.length)}
  ...
  def wordAnagrams(word: Word): List[Word] = {
    val occList = dictionaryByOccurrences get wordOccurrences(word)
    occList match {
      case Some(l) => l
      case None => List()
    }
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = {
    if (occurrences.isEmpty) List(List())
      ...
      } yield if (i != 0) (occurrences.head._1, i) :: rest else rest
    }.toList
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def occurAnagrams(occ: Occurrences): List[Sentence] = {
      if (occ.isEmpty) List(List())
        ...
        val sentCombs = combinations(occ)
        for {
          combo <- combinations(occ)
          words <- dictionaryByOccurrences(combo)
          rest <- occurAnagrams(subtract(occ, combo))
        } yield words :: rest
      }
    }
